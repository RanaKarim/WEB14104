//conect html to java on console
console.log("ready to rumble");

//use jQuery to select button
//"lsten" for the when the button is clicked
$("button").on("click", searchForRecipes)

function searchForRecipes () {

	//code for the function in curley brackets

	//create variable
    //select input using jQuery ($)
    // use the val() fucntion yo get the input value
var searchInput = $('input[type="text"]').val();
console.log(searchInput);
//alert(searchInput)

//use this function to search for recipes on edan api
...
function displayRecipes(responseJSON) {
	...
	while(counter < total ) {
		...
		//use template function to get a list item
		var li =getRecipeListItem(recipe);

        //apend the list item to html
        $("ul").append(li);
